...
interface ISystemManager {
    ...
    // Core functions
    function registerManager(string memory managerType, address managerAddress) external;

    function getManager(string memory managerType) external view returns (address);

    function updateManager(string memory managerType, address newAddress) external;
