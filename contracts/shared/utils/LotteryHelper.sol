...
contract LotteryHelper {
    ...
    function validateGameParameters(
        ...
        require(
            ticketPrice >= LotteryUtils.MIN_TICKET_PRICE,
            "Ticket price too low"
        );
        require(
            ticketPrice <= LotteryUtils.MAX_TICKET_PRICE,
            "Ticket price too high"
        );
        require(
            LotteryUtils.validateGameDuration(gameDuration),
    ...
    function validateTicketPurchase(
        ...
        require(
            LotteryUtils.validateTicketPurchase(
                ticketPrice,
                playerCount,
                maxTicketsPerPlayer,
        ...
        for (uint256 i = 0; i < ticketPrices.length; i++) {
            try
                this.validateTicketPurchase(
                    ticketPrices[i],
                    playerCounts[i],
                    maxTicketsPerPlayers[i],
                    currentPlayerTickets[i],
                    ticketCounts[i]
                )
            {
                results[i] = true;
    ...
    function calculateGameStats(
        ...
        return
            LotteryUtils.calculateGameStats(
                totalPlayers,
                totalJackpot,
                totalTickets
            );
